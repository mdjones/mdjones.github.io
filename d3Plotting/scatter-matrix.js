...
ScatterMatrix = function(dataSource, dataSourceType) {
  this.__dataSourceType = dataSourceType
...
ScatterMatrix.prototype.onData = function(cb) {
  ...
  if (this.__dataSourceType === DataSourceType.URL) {
    d3.csv(self.__dataSource, function(data) {
      self.__data = data;
      cb();
    });
  }else if(this.__dataSourceType === DataSourceType.Array) {

       self.__data = this.__dataSource;
       cb();
  }
};
  ...
  this.onData(function() {
    ...
    numeric_variables.forEach(function(trait) {
     ...
     // y[trait] = d3.scale.linear().domain(domain).range(range_y.reverse());
      x[trait] = d3.scale.linear().domain([-1,1]).range(range_x);
      y[trait] = d3.scale.linear().domain([-1,1]).range(range_y.reverse());
    });
